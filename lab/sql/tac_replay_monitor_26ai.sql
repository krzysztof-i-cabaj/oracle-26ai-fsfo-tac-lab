...
--
-- Wymagania [PL]:    - Oracle 23ai+ EE z dzialajacym TAC
--                    - (Uruchamiac na Primary lub Active Data Guard po switchover)
-- Requirements [EN]: - Oracle 23ai+ EE with active TAC
--                    - SELECT_CATALOG_ROLE
--
...
PROMPT
PROMPT (Jesli 0 rows = brak sesji z TAC context; mozliwe ze nie bylo niedawno failovera / If 0 rows = no sessions with TAC context; maybe no failover recently)
...
PROMPT
PROMPT (Jesli 0 rows = brak bledow replay w ostatnich 24h — super! / If 0 rows = no replay errors in last 24h — great!)
...
PROMPT
PROMPT (Executions > 10 w ostatnich 7 dniach; przejrzyj kod aplikacji aby przeniesc / Executions > 10 in last 7 days; review app code to move
PROMPT  zewnetrzne wywolania POZA transakcje. / external calls OUTSIDE of transactions.)
...
PROMPT
PROMPT (Jesli brak wynikow / ORA-00942 = brak Diagnostic Pack licensing / If no results / ORA-00942 = no Diagnostic Pack licensing)
...
PROMPT      PASS  = success rate >= 95% (zdrowe / healthy)
PROMPT      WARN  = 80-95% (app design issue — przejrzyj failed replays / review failed replays)
PROMPT      CRIT  = < 80% (non-replayable ops lub / or network issues)
PROMPT
PROMPT    Section 4 (alert log):
PROMPT      0 rows = zadnych bledow replay w ostatnich 24h (idealnie) / no replay errors in last 24h (ideal)
PROMPT      > 0    = przejrzyj kazdy event + koreluj z Section 5 / review each event + correlate with Section 5
PROMPT
PROMPT    Section 5 (non-replayable SQL):
PROMPT      0 rows    = app code zgodny z TAC best practices / app code compliant with TAC best practices
PROMPT      > 0 rows  = zglos app teamowi refactoring / report to app team for refactoring
PROMPT ================================================================================
