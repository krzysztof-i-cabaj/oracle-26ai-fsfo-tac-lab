...
-- Wersja:       1.0
--
-- Zmiany v1.0 (FIX-092): / Changes v1.0 (FIX-092):
--   - Bazuje na validate_environment.sql v1.0 (2026-04-23). / Based on validate_environment.sql v1.0.
--   - DLACZEGO _26ai variant (CDB-aware): w 23ai/26ai TAC services (`MYAPP_TAC`) / WHY _26ai variant (CDB-aware): in 23ai/26ai TAC services
--     sa zawsze PDB-level — zaklada sie ze CDB$ROOT nie hostuje TAC bezposrednio, / are always PDB-level — assuming CDB$ROOT does not host TAC,
--     tylko PDB-y (np. `APPPDB`). `dba_services` w CDB$ROOT pokazuje tylko CDB-level / only PDBs do. `dba_services` in CDB$ROOT shows only CDB-level
--     services -> 4 TAC checks daja 0 service(s) mimo ze MYAPP_TAC istnieje w PDB. / services -> 4 TAC checks give 0 service(s) even if MYAPP_TAC exists in PDB.
--   - Naprawa: `cdb_services WHERE con_id > 1` (PDB-only). `cdb_services` widzi / Fix: `cdb_services WHERE con_id > 1` (PDB-only).
--     services we wszystkich containers; con_id=1 to CDB$ROOT, con_id>1 = PDB. / `cdb_services` sees all containers; con_id=1 is CDB$ROOT, con_id>1 is PDB.
--   - Plus dodajemy do summary noticebox o ktorej PDB pokazuje TAC services. / Plus we add a noticebox to summary about which PDB shows TAC services.
--
...
WITH checks AS (
    ...
    SELECT
        1 AS numer,
        'FSFO'    AS kategoria,
        'Oracle version >= 23ai'        AS nazwa_check,
...
PROMPT
PROMPT (Jesli 0 wierszy = brak skonfigurowanych TAC services w zadnym PDB. / If 0 rows = no TAC services configured in any PDB.)
PROMPT  Po doc 12 (deploy_tac_service.sh) powinno byc widac MYAPP_TAC w APPPDB / After doc 12 (deploy_tac_service.sh) MYAPP_TAC should be visible in APPPDB
PROMPT  z failover_type=TRANSACTION, commit_outcome=YES, ssc=DYNAMIC, FAN=YES. / with failover_type=TRANSACTION, commit_outcome=YES, ssc=DYNAMIC, FAN=YES.
PROMPT  UWAGA: srvctl pokazuje 'Commit Outcome: TRUE' ale dictionary view zwraca YES/NO. / NOTE: srvctl shows 'Commit Outcome: TRUE' but dictionary view returns YES/NO.)
...
PROMPT   - PASS  = srodowisko gotowe do wdrozenia FSFO/TAC / environment ready for FSFO/TAC deployment
PROMPT   - WARN  = dziala, ale zalecana poprawa przed produkcja / working, but improvement recommended before production
PROMPT   - FAIL  = blokuje wdrozenie, musi byc naprawione / blocks deployment, must be fixed
PROMPT   - N/A   = nie dotyczy (np. TAC checks na bazie bez services) / not applicable (e.g. TAC checks on DB without services)
PROMPT
PROMPT Minimalne wymaganie dla Go-Live: wszystkie FSFO checks PASS, wszystkie TAC checks PASS lub WARN. / Minimum requirement for Go-Live: all FSFO checks PASS, all TAC checks PASS or WARN.
PROMPT
PROMPT Zakres TAC checks: cdb_services WHERE con_id > 1 (PDB-level). CDB$ROOT services (con_id=1) ignorowane. / TAC checks scope: cdb_services WHERE con_id > 1 (PDB-level). CDB$ROOT services ignored.
PROMPT

PROMPT
PROMPT ================================================================================
PROMPT  Validation complete (26ai CDB-aware variant). Plik raportu / Report file: reports/{DB}_validation.txt
PROMPT ================================================================================
