-- ==============================================================================
-- Tytul:        tac_full_readiness.sql
-- Opis:         Pelny readiness check TAC przed wdrozeniem (12 sekcji):
--               DB tier + Service tier + Network tier + TG + UCP hints
-- Description [EN]: Full TAC readiness check before deployment (12 sections):
--                   DB tier + Service tier + Network tier + TG + UCP hints
--
-- Autor:        KCB Kris
-- Data:         2026-04-23
...
SELECT 'Oracle version (wymagane 19c+)'              AS sprawdzenie,
       SUBSTR(bf, 1, 40)                             AS wartosc,
       '19c / 21c / 23ai / 26ai'                     AS oczekiwane,
       CASE WHEN bf LIKE '%19.%' OR bf LIKE '%21.%'
              OR bf LIKE '%23.%' OR bf LIKE '%26.%'
...
FROM (
    ...
    SELECT name,
           ...
           commit_outcome,
           'TRUE',
           CASE WHEN commit_outcome = 'TRUE' THEN 'PASS' ELSE 'FAIL' END,
           2
    FROM   tac_candidates
    UNION ALL
    SELECT name,
    ...
    SELECT name,
           ...
           aq_ha_notifications,
           'TRUE',
           CASE WHEN aq_ha_notifications = 'TRUE' THEN 'PASS' ELSE 'FAIL' END,
           6
    FROM   tac_candidates
    UNION ALL
    SELECT name,
...
PROMPT
PROMPT (Jesli 0 rows = dobrze; inaczej przegladaj kod aplikacji)
...
PROMPT
PROMPT (Dla KAZDEGO app usera oczekiwane 3 wiersze: KEEP DATE TIME, KEEP SYSGUID, KEEP ANY SEQUENCE
PROMPT  Jesli brakuje - DBMS_APP_CONT_ADMIN nie zachowa mutable values podczas replay)
...
PROMPT
PROMPT (0 wierszy = dobrze. Inaczej: ALTER SEQUENCE ... CACHE 1000 NOORDER
PROMPT  oraz rozwazyc GRANT KEEP ANY SEQUENCE dla replay)
...
PROMPT
PROMPT (Oczekiwane: client_version >= 19.0 dla TAC; ojdbc11.jar lub ojdbc8.jar 19c+)

-- ============================================================================
-- SEKCJA 11: Replay statistics (jesli byly uzywane)
-- Section 11: Replay statistics (if used)
-- ============================================================================

PROMPT
PROMPT --------------------------------------------------------------------------------
PROMPT  SEKCJA 11 / SECTION 11: Replay statistics (GV$REPLAY_STAT_SUMMARY)
PROMPT --------------------------------------------------------------------------------

COLUMN inst_id           FORMAT 999 HEADING "Inst"
COLUMN requests_total    FORMAT 99999999 HEADING "Wszystkie"
COLUMN requests_replayed FORMAT 99999999 HEADING "OK"
COLUMN requests_failed   FORMAT 99999999 HEADING "Fail"
COLUMN pct_success       FORMAT 999.9 HEADING "Succ %"
...
PROMPT
PROMPT ================================================================================
PROMPT  TAC Full Readiness check zakonczony.
PROMPT  Wszystkie PASS -> srodowisko gotowe do wdrozenia TAC.
