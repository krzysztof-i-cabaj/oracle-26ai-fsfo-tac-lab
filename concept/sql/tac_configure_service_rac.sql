...
-- Wersja:       1.0
--
-- Wymagania [PL]:    - Oracle 19c+ EE, RAC
--                    - Uprawnienia DBA (lub EXECUTE na DBMS_SERVICE)
-- Requirements [EN]: - Oracle 19c+ EE, RAC
...
COLUMN failover_method         FORMAT A10 HEADING "Method"
...
SELECT
    ...
    aq_ha_notifications
FROM   dba_services
WHERE  name IN ('&service_rw', '&service_ro')
    OR name NOT LIKE 'SYS%';

PROMPT
PROMPT Aktualnie zdefiniowane service'y (dba_services - filtrowane aplikacyjne):
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('-- === 1. TAC service (RW, role PRIMARY) ===');
...
PROMPT   SELECT name, failover_type, commit_outcome, retention_timeout,
PROMPT          session_state_consistency, drain_timeout, aq_ha_notifications
PROMPT   FROM   dba_services
PROMPT   WHERE  name = '&service_rw';
PROMPT
PROMPT Oczekiwane wartosci / Expected values:
PROMPT   failover_type              = TRANSACTION
PROMPT   commit_outcome             = TRUE
PROMPT   retention_timeout          = 86400
PROMPT   session_state_consistency  = DYNAMIC
PROMPT   drain_timeout              = 300
PROMPT   aq_ha_notifications        = TRUE
